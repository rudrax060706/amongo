...
async def handle_base_bid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not submission_id:
        await update.message.reply_text("⚠️ Something went wrong — submission not found. Please try /add again.")
        return
    ...
    log_caption = (
        ...
        f"🆔 <b>Item ID:</b> <code>{submission_id}</code>\n"
        f"👤 <b>Name:</b> {user.first_name}\n"
        f"🔗 <b>Username:</b> @{user.username if user.username else 'N/A'}\n"
        f"🎬 <b>Anime:</b> {context.user_data.get('anime_name', 'Unknown')}\n"
        f"💞 <b>{context.user_data['type'].capitalize()}:</b> {context.user_data.get('waifu_name', 'Unknown')}\n"
        f"💎 <b>Rarity:</b> {RARITY_MAP.get(context.user_data['rarity'])} {context.user_data['rarity']}\n"
        f"💰 <b>Base Bid:</b> {base_bid}\n"
        f"🏷️ <b>Tag:</b> {context.user_data.get('optional_tag', '-')}\n"
